...
string readBarcode(Mat img,string last)
{
   ...
   for(Image::SymbolIterator symbol = image.symbol_begin();  
     ...
     // do something useful with results  
			if(last==symbol->get_data()){return last;}
			else if(first)
			{
				last=symbol->get_data();
				first=false;
			}
			cout << "Barcode mode:" << symbol->get_type_name()<<endl  
			<< "Barcode data:" << symbol->get_data() << '"' <<" "<< endl;
			cout<<rand()<<endl;
